...
export default function CharacterPage() {
  ...
  const derivedStats = useMemo(() => {
    ...
    const fromEquipment = equippedIds.reduce(
      (acc, instanceId) => {
  ...
  const handlePendingChange = (stat: 'a' | 'b' | 'c' | 'd', delta: number) => {
    ...
    if (delta > 0 && totalPending >= unspentPoints) return;
    if (delta < 0 && pendingUpgrades[stat] <= 0) return;
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {char.equipment &&
              Object.entries(char.equipment).map(([slot, instanceId]) => (
                <EquipRow
                  key={slot}
                  slotName={slot}
...
function EquipRow({ slotName, equippedId, gameItems, inventory, onUnequip }: any) {
  const instance = equippedId && inventory ? inventory.find((i: any) => i.instanceId === equippedId) : null;
  ...
  return (
    <div className={`border-2 rounded-lg relative overflow-hidden ${isEquipped ? 'border-blue-500' : 'border-gray-700'}`}>
      ...
      <div className="p-2 bg-gray-700/80">
        ...
        {isEquipped && <p className="text-xs text-center text-blue-400 mt-1 font-bold">Equipped</p>}
        {isBroken && <p className="text-xs text-center text-red-500 mt-1 font-bold">Broken</p>}
      </div>
    </div>
  );
}
