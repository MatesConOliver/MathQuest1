...
import { 
  ...
  StoryEvent, StoryTrigger, StoryScene
} from "@/types/game";
...
export default function GMPage() {
  ...
  const [activeTab, setActiveTab] = useState<"questions" | "foes" | "encounters" | "items" | "locations" | "stories">("questions");
...
function StoriesPanel() {
  ...
  async function loadStories() {
    try {
      ...
      const snap = await getDocs(q);
      setStories(snap.docs.map(d => ({ ...d.data(), id: d.id } as StoryEvent)));
    } catch (e: any) { setMsg("Error loading stories: " + e.message); }
  }

  function resetForm() {
    setEditingId("");
    setId(""); setTitle(""); setTriggerType("ON_LOGIN"); setTriggerCondition(""); setOneTime(true);
    setScenes([]);
    setRewardXp(0); setRewardGold(0); setUnlockMapId("");
    setMsg("");
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700">
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-bold">{editingId ? "✏️ Edit Story" : "📖 New Story"}</h2>
          <button onClick={resetForm} className="text-xs underline text-gray-500">Reset</button>
        </div>
        
        {msg && <div className="p-2 bg-blue-100 text-blue-800 rounded text-center text-sm font-bold">{msg}</div>}

        <div className="grid grid-cols-2 gap-4">
          <Input label="Story ID (Unique)" value={id} onChange={(e:any) => setId(e.target.value)} disabled={!!editingId} />
          <Input label="Title (Internal)" value={title} onChange={(e:any) => setTitle(e.target.value)} />
        </div>

        <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl space-y-3 border dark:border-gray-600">
           <h3 className="text-xs font-bold uppercase text-gray-400">Trigger Logic</h3>
           <div className="grid grid-cols-2 gap-4">
             <div>
                <label className="text-xs font-bold text-gray-500 uppercase">Trigger Type</label>
                <select 
                ...
                >
                  ...
                  <option value="ON_ENTER_MAP">ON_ENTER_MAP (Specific Location)</option>
                  ...
                  <option value="ON_OBJECT_CONDITIONS">ON_OBJECT_CONDITIONS</option>
                </select>
             </div>
             <Input label="Condition ID (e.g. 'rat_king')" value={triggerCondition} onChange={(e:any) => setTriggerCondition(e.target.value)} />
           </div>
        ...
        <div className="space-y-2">
           <div className="flex justify-between items-end">
              ...
              <button onClick={addScene} className="text-xs bg-black text-white px-2 py-1 rounded hover:bg-gray-800">
                + Add Scene
              </button>
           </div>
           ...
           <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 border-t pt-4 dark:border-gray-700">
              {scenes.map((scene, idx) => (
                <div key={idx} className="p-3 border rounded-xl bg-gray-50 dark:bg-gray-900/30 dark:border-gray-600 space-y-2 relative">
                   <div className="absolute top-2 right-2">
                      <button onClick={() => deleteScene(idx)} className="text-red-400 hover:text-red-600 font-bold text-xs">✕</button>
                   </div>
                   
                   <div className="grid grid-cols-3 gap-2">
                      <Input label="Speaker" value={scene.speakerName} onChange={(e:any) => updateScene(idx, 'speakerName', e.target.value)} />
                      <div className="col-span-2">
                        <Input label="Sprite URL (Optional)" value={scene.speakerSprite} onChange={(e:any) => updateScene(idx, 'speakerSprite', e.target.value)} />
                      </div>
                   </div>
                   
                   <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">Dialogue Text</label>
                      <textarea 
                   ...
                   <div className="grid grid-cols-2 gap-2">
                      <Input label="Background URL" value={scene.backgroundUrl} onChange={(e:any) => updateScene(idx, 'backgroundUrl', e.target.value)} />
                      <Input label="Music URL" value={scene.musicUrl} onChange={(e:any) => updateScene(idx, 'musicUrl', e.target.value)} />
                   </div>
                   
                   <div className="pt-2 border-t dark:border-gray-700">
                      <Input label="Next Scene ID (Leave empty for 'Next in List')" value={scene.nextSceneId} onChange={(e:any) => updateScene(idx, 'nextSceneId', e.target.value)} />
                   </div>
                </div>
              ))}
              {scenes.length === 0 && <p className="text-gray-400 text-sm text-center">No scenes yet.</p>}
           </div>
        </div>

        {/* REWARDS */}
        <div className="p-4 bg-yellow-50 dark:bg-yellow-900/10 rounded-xl border border-yellow-100 dark:border-yellow-800">
            <h3 className="text-xs font-bold uppercase text-yellow-600 mb-2">Completion Rewards</h3>
            <div className="grid grid-cols-3 gap-2">
               <Input label="XP" type="number" value={rewardXp} onChange={(e:any) => setRewardXp(Number(e.target.value))} />
               <Input label="Gold" type="number" value={rewardGold} onChange={(e:any) => setRewardGold(Number(e.target.value))} />
               <Input label="Unlock Map ID" value={unlockMapId} onChange={(e:any) => setUnlockMapId(e.target.value)} />
            </div>
        </div>

        <button onClick={saveStory} className="w-full py-3 bg-black text-white dark:bg-white dark:text-black rounded-xl font-bold text-lg hover:opacity-90">
           💾 Save Story Event
        </button>
      ...
      <div className="border-l pl-6 dark:border-gray-700 space-y-4">
        <h3 className="font-bold text-gray-400 uppercase text-xs">Existing Stories</h3>
        <input 
        ...
        <div className="space-y-2 h-[600px] overflow-y-auto">
          {stories
            ...
            .map(s => (
            <div key={s.id} onClick={() => loadStoryToEdit(s)} className={`p-3 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors ${editingId === s.id ? 'border-black ring-1 ring-black dark:border-white' : 'dark:border-gray-600'}`}>
               <div className="flex justify-between items-start">
                 <div>
                   <div className="font-bold text-sm">{s.title}</div>
                   <div className="text-xs text-gray-500 font-mono mt-1">{s.id}</div>
                 </div>
                 <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase ${s.oneTime ? 'bg-purple-100 text-purple-600' : 'bg-green-100 text-green-600'}`}>
                    {s.triggerType}
                 </span>
               </div>
               ...
               <button onClick={(e) => { e.stopPropagation(); deleteStory(s.id); }} className="mt-2 text-red-400 hover:text-red-600 text-xs font-bold underline">
                  Delete
               </button>
            </div>
          ))}
